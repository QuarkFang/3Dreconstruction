...
def compute(image):
    ...
    descriptors = surf.compute(gray, keypoints)
...
def show_line(image, result):
    for i in range(len(result)):
        cv2.line(image, (result[i][0], result[i][1]), (int(result[i][2] + 1000), result[i][3]), (255, 255, 0), 1)
    # 将关键点在图像上显示
...
# 特征点匹配
def match_surf(location1, location2, descriptors1, descriptors2, DR = 0.6):
    loc = []
    for i in range(len(location1)):
        ...
        for j in range(len(location2)):
            ...
            if length < l1:
                ...
                l2 = length
        if l1/l2 < DR:
            loc.append([round(location1[i][0]), round(location1[i][1]), round(location2[location2_l1][0]), round(location2[location2_l1][1])])
            print([[l1,l2]])
        if i%100 == 0:
